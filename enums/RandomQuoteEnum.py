...
class RandomQuoteEnum(Enum):
    ...
    CAUCH = RandomQuote(4, "u r naked on your cauch, having sex with your boyfriend. i came by knocked your door not very loud. u didnt hear it, and i open oyur door and find u naked on the couch with a dick inside u. i am very sure u would be  very okish with that. u wouldnt throw the couch on my head but anw. i personally dont like this at all ", EmojiEnum.WISENAKIS, False)

    MEXIOCRITY = RandomQuote(5, "I really dont see the diference  etween simplicity , cause in the end of the day they both helong in the big picture whicth u cant just see but sure, if u want to call it mexiocrity, then ok its mediocrity", EmojiEnum.WISENAKIS, False)

    ANTONIO = RandomQuote(6, "antonio frena!  frena frena!! antonio!! frena!! ANTONIO FRENA!!!", EmojiEnum.WISENAKIS, False)

    ANCHIEVE = RandomQuote(7, "I am gianniscand i am proud to call me a happy person ( or mediorce  as u would call it) that spent my life in missery cause i will always strive for something i will never anchieve, perfection", EmojiEnum.WISENAKIS, False)

    DELICANCY = RandomQuote(8, "sucha  complicate delicancy, not chocolates simplicity", EmojiEnum.WISENAKIS, False)

    X_D = RandomQuote(9, "also, sibl corrected 1 of my mistakes, whitch means she readed it - and if sibl ignored it/ didnt got triggered enough by it, it means its not that bad xD", EmojiEnum.WISENAKIS, False)

    PROSTITUE = RandomQuote(10, "u are a prostitute and u sell your body? i dont see it asa human. i see it as an object.  in this world when u pay for THINGS, u get things. u wanna sell your body? your body now is a thing, its an item an item that i booked down for the next 30m. u want to sell service? fullfill your services", EmojiEnum.WISENAKIS, False)

    PUSSY = RandomQuote(11, "assets/images/pussy.png", EmojiEnum.WISENAKIS, True)

    PANGEA = RandomQuote(12, "i know what pangea is... u know what. fuck this shit... i am not gonna bother... sure ok u r right it feels like u dont even bother to read what i say, thats what getting me upset.... i understant , maybe its  alanguage barrier again- thats why i am like just let it go", EmojiEnum.WISENAKIS, False)

    EXPERTISE = RandomQuote(13, "but again. u testing my expertise on the english language. if u judge me over 1 misshear, imo thats wrong. cause that has nothing to do with  my expertise in the language. if during the whole interview i was like: what? could u repeat that? then yes", EmojiEnum.WISENAKIS, False)

    PLANTS = RandomQuote(14, "ah! i see, its sinep in reverse. makes sense why plants are green then", EmojiEnum.WISENAKIS, False)

    @staticmethod
    def get_random_quote(ctx):
        index = randrange(len(RandomQuoteEnum))
        return RandomQuoteEnum.get_quote(ctx, index)

    @staticmethod
    async def get_quote(ctx, id):
        ...
        if quote == RandomQuoteEnum.RANDOM_RANDOM:
            ...
            for channel in ctx.guild.text_channels:
                ...
                async for message in channel.history(limit=101, around=random_start):
                    if message.author.id == UserEnum.GIANNAKIS.value.id:
